...
const roles = ['SYSTEM_ADMIN', 'OWNER', 'BRANCH_MANAGER', 'FRONT_DESK', 'TECHNICIAN'] as const;
...
const paymentMethods = ['CASH', 'CARD', 'UPI', 'BANK_TRANSFER', 'OTHER'] as const;
const feasibilities = ['REPAIRABLE', 'PARTIALLY_REPAIRABLE', 'UNREPAIRABLE', 'FURTHER_TESTING_REQUIRED'] as const;
...
export const createTicketSchema = z.object({
  ...
  reportedProblem: z.string().min(5, 'Please provide a clear problem statement (min 5 chars)'),
...
export const createDiagnosisSchema = z.object({
  ...
  recommendedRepair: z.string().min(5, 'Recommended repair details are required'),
...
export const estimateItemSchema = z.object({
  ...
  unitPrice: z.number().int().nonnegative('Unit price must be a non-negative integer (cents)'),
});

export const createEstimateSchema = z.object({
  items: z.array(estimateItemSchema).min(1, 'Estimate must contain at least one item'),
