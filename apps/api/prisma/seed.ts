import { PrismaClient, UserRole, UserStatus, TicketPriority, TicketStatus, RepairFeasibility, EstimateStatus, EstimateItemType, InvoiceStatus, PaymentMethod, AttachmentCategory } from '@prisma/client';

const prisma = new PrismaClient();

// Argon2id hash of 'password123'
const DEV_PASSWORD_HASH = '$argon2id$v=19$m=19456,t=2,p=1$nGiFKTYIPvCfbr4rppf3zQ$zipOoL2tLP2/U2GjEm27W+uYM0SUj+MMCp2G6lJAeUU';
const PASSWORD_HASH = '$argon2id$v=19$m=19456,t=2,p=1$nGiFKTYIPvCfbr4rppf3zQ$zipOoL2tLP2/U2GjEm27W+uYM0SUj+MMCp2G6lJAeUU';
...
async function main() {
  ...
  const customerNames = [
    ...
    { name: 'Patricia Anderson', phone: '+15550010', email: 'patricia@anderson.net' },
  ];
  ...
  // 6. Seed Devices
  const deviceList = [
    { customerIndex: 0, category: 'Mobile phone', brand: 'Apple', model: 'iPhone 15 Pro Max', serial: 'S15PM123456', imei: '351234567890123' },
    { customerIndex: 0, category: 'Tablet', brand: 'Apple', model: 'iPad Air (M1)', serial: 'SIPA987654', imei: null },
    { customerIndex: 1, category: 'Laptop', brand: 'Dell', model: 'XPS 15 9530', serial: 'SDXPS112233', imei: null },
    { customerIndex: 2, category: 'Mobile phone', brand: 'Samsung', model: 'Galaxy S24 Ultra', serial: 'SS24U556677', imei: '359876543210987' },
    { customerIndex: 3, category: 'Laptop', brand: 'Apple', model: 'MacBook Pro 16" M3 Max', serial: 'SMBP998877', imei: null },
    { customerIndex: 4, category: 'Gaming console', brand: 'Sony', model: 'PlayStation 5 Slim', serial: 'SPS5001122', imei: null },
    { customerIndex: 5, category: 'Television', brand: 'LG', model: 'OLED55C3PSA', serial: 'SLGOLED5566', imei: null },
    { customerIndex: 6, category: 'Camera', brand: 'Sony', model: 'Alpha 7 IV', serial: 'SSONYA74433', imei: null },
    { customerIndex: 7, category: 'Desktop computer', brand: 'Custom', model: 'Ryzen 7 Gaming PC', serial: 'SPC990011', imei: null },
    { customerIndex: 8, category: 'Mobile phone', brand: 'Google', model: 'Pixel 8 Pro', serial: 'SPIXEL8P99', imei: '350011223344556' },
    { customerIndex: 9, category: 'Laptop', brand: 'Lenovo', model: 'ThinkPad X1 Carbon Gen 11', serial: 'SLTPX19988', imei: null },
    { customerIndex: 9, category: 'Tablet', brand: 'Samsung', model: 'Galaxy Tab S9 Ultra', serial: 'SSTABS9887', imei: null },
  ];
  ...
  const statusesList = [
    { status: TicketStatus.RECEIVED, customerIdx: 0, deviceIdx: 0, branch: branchA, creator: frontDeskA, tech: null },
    { status: TicketStatus.RECEIVED, customerIdx: 1, deviceIdx: 2, branch: branchB, creator: frontDeskB, tech: null },
    { status: TicketStatus.DIAGNOSING, customerIdx: 2, deviceIdx: 3, branch: branchA, creator: frontDeskA, tech: techA1 },
    { status: TicketStatus.DIAGNOSING, customerIdx: 3, deviceIdx: 4, branch: branchB, creator: frontDeskB, tech: techB1 },
    { status: TicketStatus.WAITING_FOR_APPROVAL, customerIdx: 4, deviceIdx: 5, branch: branchA, creator: frontDeskA, tech: techA2 },
    { status: TicketStatus.WAITING_FOR_APPROVAL, customerIdx: 5, deviceIdx: 6, branch: branchB, creator: frontDeskB, tech: techB2 },
    { status: TicketStatus.APPROVED, customerIdx: 6, deviceIdx: 7, branch: branchA, creator: frontDeskA, tech: techA1 },
    { status: TicketStatus.APPROVED, customerIdx: 7, deviceIdx: 8, branch: branchB, creator: frontDeskB, tech: techB1 },
    { status: TicketStatus.REPAIR_IN_PROGRESS, customerIdx: 8, deviceIdx: 9, branch: branchA, creator: frontDeskA, tech: techA2 },
    { status: TicketStatus.REPAIR_IN_PROGRESS, customerIdx: 9, deviceIdx: 10, branch: branchB, creator: frontDeskB, tech: techB2 },
    { status: TicketStatus.READY_FOR_COLLECTION, customerIdx: 0, deviceIdx: 1, branch: branchA, creator: frontDeskA, tech: techA1 },
    { status: TicketStatus.READY_FOR_COLLECTION, customerIdx: 9, deviceIdx: 11, branch: branchB, creator: frontDeskB, tech: techB1 },
    { status: TicketStatus.DELIVERED, customerIdx: 1, deviceIdx: 2, branch: branchA, creator: frontDeskA, tech: techA2 },
    { status: TicketStatus.DELIVERED, customerIdx: 2, deviceIdx: 3, branch: branchB, creator: frontDeskB, tech: techB2 },
    { status: TicketStatus.REJECTED, customerIdx: 3, deviceIdx: 4, branch: branchA, creator: frontDeskA, tech: techA1 },
    { status: TicketStatus.UNREPAIRABLE, customerIdx: 4, deviceIdx: 5, branch: branchB, creator: frontDeskB, tech: techB1 },
    { status: TicketStatus.PARTS_REQUIRED, customerIdx: 5, deviceIdx: 6, branch: branchA, creator: frontDeskA, tech: techA2 },
    { status: TicketStatus.CANCELLED, customerIdx: 6, deviceIdx: 7, branch: branchB, creator: frontDeskB, tech: null },
    { status: TicketStatus.REPAIR_IN_PROGRESS, customerIdx: 7, deviceIdx: 8, branch: branchA, creator: frontDeskA, tech: techA1 },
    { status: TicketStatus.READY_FOR_COLLECTION, customerIdx: 8, deviceIdx: 9, branch: branchB, creator: frontDeskB, tech: techB2 },
  ];
  ...
  for (const t of statusesList) {
    ...
    const ticket = await prisma.repairTicket.create({
      data: {
        ...
        diagnosisStartedAt: (t.status as any) !== TicketStatus.RECEIVED ? new Date() : null,
        repairStartedAt: ([TicketStatus.REPAIR_IN_PROGRESS, TicketStatus.READY_FOR_COLLECTION, TicketStatus.DELIVERED] as any[]).includes(t.status) ? new Date() : null,
        readyAt: ([TicketStatus.READY_FOR_COLLECTION, TicketStatus.DELIVERED] as any[]).includes(t.status) ? new Date() : null,
        completedAt: ([TicketStatus.READY_FOR_COLLECTION, TicketStatus.DELIVERED] as any[]).includes(t.status) ? new Date() : null,
        deliveredAt: t.status === TicketStatus.DELIVERED ? new Date() : null,
        deliveredById: t.status === TicketStatus.DELIVERED ? t.creator.id : null,
        deliveryNotes: t.status === TicketStatus.DELIVERED ? 'Delivered to customer in person.' : null,
      },
    });
    ...
    if (t.tech) {
      ...
      if (!([TicketStatus.RECEIVED, TicketStatus.DIAGNOSING] as any[]).includes(t.status)) {
        const diag = await prisma.diagnosis.create({
          data: {
            repairTicketId: ticket.id,
            technicianId: t.tech.id,
            faultCategory: 'Power Management IC',
            diagnosticFindings: 'Found PMIC short circuit causing power failure.',
            ...
            publicExplanation: 'Main power chip has failed and needs replacement.',
        ...
        const estStatus = ([TicketStatus.WAITING_FOR_APPROVAL] as any[]).includes(t.status)
          ? EstimateStatus.SENT
          : ([TicketStatus.REJECTED, TicketStatus.CANCELLED] as any[]).includes(t.status)
        ...
        const est = await prisma.estimate.create({
          data: {
            ...
            customerNotes: 'Please approve the estimate to proceed with the repair.',
            internalNotes: 'Cost of PMIC: $20, Labour margin: $100',
            createdById: t.creator.id,
            sentAt: new Date(),
            approvedAt: estStatus === EstimateStatus.APPROVED ? new Date() : null,
            rejectedAt: estStatus === EstimateStatus.REJECTED ? new Date() : null,
          },
        });
        ...
        await prisma.estimateDecision.create({
          data: {
            estimateId: est.id,
            decision: estStatus === EstimateStatus.REJECTED ? 'REJECTED' : 'APPROVED',
            tokenHash: `token-hash-simulation-${est.id}`,
            tokenExpiresAt: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000),
            decisionAt: estStatus === EstimateStatus.APPROVED || estStatus === EstimateStatus.REJECTED ? new Date() : null,
            customerComment: estStatus === EstimateStatus.APPROVED ? 'Please fix as soon as possible' : 'Too expensive',
            ipAddress: '127.0.0.1',
        ...
        if (([TicketStatus.READY_FOR_COLLECTION, TicketStatus.DELIVERED] as any[]).includes(t.status)) {
          ...
          const isPaid = t.status === TicketStatus.DELIVERED ? InvoiceStatus.PAID : InvoiceStatus.UNPAID;
  ...
  console.log('Seeded repair tickets, status history, estimates, invoices, and notifications.');
