...
describe('AuthService', () => {
  ...
  describe('validateUser', () => {
    it('should throw UnauthorizedException if user does not exist', async () => {
      ...
      await expect(service.validateUser('none@user.com', 'pwd')).rejects.toThrow(
    ...
    it('should throw ForbiddenException if user is locked out', async () => {
      ...
      mockPrismaService.user.findUnique.mockResolvedValue({ ...mockUser, lockedUntil: lockedTime });
    ...
    it('should increment failed attempts and lockout after 5 fails', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({ ...mockUser, failedLoginAttempts: 4 });
      (verify as jest.Mock).mockResolvedValue(false);

      await expect(service.validateUser(mockUser.email, 'wrong_pwd')).rejects.toThrow(
        UnauthorizedException,
      );
    ...
    it('should reset failed attempts and return user profile on success', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({ ...mockUser, failedLoginAttempts: 3 });
  ...
  describe('login', () => {
    it('should generate access token and return profile data', async () => {
      mockPrismaService.refreshSession.create.mockResolvedValue({ id: 'sess-uuid' });

      const result = await service.login(mockUser.id, mockUser.email, mockUser.role, '127.0.0.1', 'jest-agent');
      expect(result).toBeDefined();
