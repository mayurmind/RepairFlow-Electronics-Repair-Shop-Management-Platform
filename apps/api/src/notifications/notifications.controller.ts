import { Controller, Get, Patch, Delete, Param, Query, UseGuards, ParseBoolPipe } from '@nestjs/common';
...
export class NotificationsController {
  ...
  async findAll(
    ...
    const notifications = await this.notificationsService.findAll(actor.id, isRead);
  ...
  async markAsRead(@CurrentUser() actor: any, @Param('id') id: string) {
    const notification = await this.notificationsService.markAsRead(id, actor.id);
