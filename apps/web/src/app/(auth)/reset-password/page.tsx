...
function ResetPasswordForm() {
  ...
  const onSubmit = async (data: any) => {
    if (!token) {
      setErrorMsg('Token is missing from URL. Please use the complete reset link.');
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-premium p-8 relative overflow-hidden">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <h2 className="font-bold text-2xl tracking-tight text-slate-900 font-outfit">Choose New Password</h2>
        <p className="text-xs text-slate-500 mt-1">Please enter a strong and secure password</p>
      </div>
        ...
        <div className="mb-6 flex items-start gap-2.5 bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-xl text-sm">
          ...
          <div>
            ...
            <p className="text-xs mt-0.5">
              The URL does not contain a password reset token. Please check the reset link and try again.
            </p>
          </div>
        </div>
      )}
      ...
      {success ? (
        <div className="text-center py-4">
          ...
          <h3 className="font-bold text-lg text-slate-900 font-outfit">Password Updated</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            ...
            />
            {errors.password && (
              <p className="text-xs text-red-600 mt-1.5 font-medium">{errors.password.message as string}</p>
            )}
          </div>
      ...
      <Suspense
        fallback={
          <div className="w-full max-w-md bg-white rounded-2xl border border-slate-200 p-8 flex items-center justify-center gap-3">
            ...
            <span className="text-sm text-slate-500 font-medium">Loading token...</span>
          </div>
        }
      >
        <ResetPasswordForm />
