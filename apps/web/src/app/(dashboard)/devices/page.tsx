    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 bg-white border border-slate-200 rounded-2xl">
        ...
        <p className="text-sm text-slate-400 mt-1 max-w-sm text-center">
          Technicians are restricted from browsing general device asset indexes
          directly.
        </p>
      </div>
    );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Register and inspect client hardware assets and model metrics.
          </p>
        </div>
        <button
      ...
      {loadingDevices ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {devicesList.map((device: any) => (
            ...
            >
              ...
              >
                <History className="w-3.5 h-3.5 text-slate-400" /> View Repair
                History
              </button>
            </div>
          ))}
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-sm text-slate-400 mt-1">
            Add devices under customer profiles to start repair logging.
          </p>
        </div>
      )}
              ...
              {loadingHistory ? (
                ...
                <div className="space-y-4">
                  {deviceHistory.data.map((ticket: any) => (
                    <div
                      key={ticket.id}
                      className="p-4 bg-slate-50 border border-slate-100 rounded-xl space-y-2 text-xs font-semibold text-slate-700"
                    >
                      <div className="flex justify-between items-center">
                        <span className="font-mono text-slate-800 font-extrabold">
                          {ticket.ticketNumber}
                        </span>
                        <span className="bg-blue-50 text-blue-800 border border-blue-100 px-2 py-0.5 rounded font-extrabold">
                          {ticket.status}
                        </span>
                      </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  {createForm.formState.errors.brand && (
                    <span className="text-red-500 text-xs mt-1 block">
                      {createForm.formState.errors.brand.message}
                    </span>
                  )}
                </div>
                <div>
