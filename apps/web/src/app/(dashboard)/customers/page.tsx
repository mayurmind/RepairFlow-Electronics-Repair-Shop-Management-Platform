...
export default function CustomersPage() {
  ...
  const createCustomerMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error(err.message || 'Failed to register customer. Check unique constraints.');
    },
  });
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 bg-white border border-slate-200 rounded-2xl">
        ...
        <p className="text-sm text-slate-400 mt-1 max-w-sm text-center">
          Technicians are restricted from browsing the customer database directly. Please search within tickets.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 h-[calc(100vh-8.5rem)] flex flex-col">
      ...
      <div className="flex justify-between items-center shrink-0">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight font-outfit text-slate-900">Customer Directory</h1>
          <p className="text-sm text-slate-500 mt-1">Manage client profiles, contact metrics, and device assignments.</p>
        </div>
        <button
              ...
              customersList.map((c: any) => (
                ...
                >
                  <div className="overflow-hidden pr-2">
                    <span className="font-bold text-sm block truncate">{c.fullName}</span>
                    <span className={`text-[10px] block font-medium font-mono mt-0.5 ${
                      selectedCustomerId === c.id ? 'text-slate-300' : 'text-slate-400'
                    }`}>
                      {c.phone}
                    </span>
                  </div>
                  <User className={`w-4 h-4 shrink-0 ${
                    selectedCustomerId === c.id ? 'text-blue-400' : 'text-slate-300'
                  }`} />
                </button>
              ))
            ) : (
        ...
        <div className="flex-1 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm overflow-y-auto">
          {loadingDetails ? (
            ...
            <div className="space-y-8">
              ...
              <div className="flex justify-between items-start pb-6 border-b border-slate-100">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold font-outfit text-slate-900">{activeCustomer.fullName}</h2>
                    <span className="text-xs text-slate-400 block font-mono mt-0.5">ID: {activeCustomer.id.slice(0, 8)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm font-semibold">
                <div className="flex items-center gap-3.5 bg-slate-50 p-4 rounded-xl border border-slate-100">
                  ...
                  <div>
                    ...
                    {activeCustomer.alternatePhone && (
                      <span className="text-xs text-slate-400 block mt-0.5">Alt: {activeCustomer.alternatePhone}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3.5 bg-slate-50 p-4 rounded-xl border border-slate-100 md:col-span-2">
                  ...
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider">Postal / Address Line</span>
                    <span className="font-normal text-slate-700">{activeCustomer.address || 'No address registered.'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                  <h3 className="font-bold text-slate-900 font-outfit flex items-center gap-2">
                    <Smartphone className="w-4 h-4 text-slate-400" /> Registered Devices
                  </h3>
                  <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                    {activeCustomer.devices && activeCustomer.devices.length > 0 ? (
                      activeCustomer.devices.map((d: any) => (
                        <div key={d.id} className="p-3 bg-slate-50 border border-slate-100 rounded-xl flex justify-between items-center text-xs font-semibold">
                          <div>
                            <span className="block text-slate-800 font-extrabold">{d.brand} {d.model}</span>
                            <span className="block text-[10px] text-slate-400 font-mono mt-0.5">SN: {d.serialNumber || 'N/A'}</span>
                          </div>
                          <span className="text-[9px] bg-slate-200 text-slate-600 px-2 py-0.5 rounded uppercase">{d.category}</span>
                        </div>
                      ))
                    ) : (
                ...
                <div className="border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                  <h3 className="font-bold text-slate-900 font-outfit flex items-center gap-2">
                    <Clipboard className="w-4 h-4 text-slate-400" /> Repair History
                  </h3>
                  <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                    {activeCustomer.repairTickets && activeCustomer.repairTickets.length > 0 ? (
                      activeCustomer.repairTickets.map((t: any) => (
                        <div key={t.id} className="p-3 bg-slate-50 border border-slate-100 rounded-xl flex justify-between items-center text-xs font-semibold">
                          <div>
                            <span className="block text-slate-800 font-extrabold font-mono">{t.ticketNumber}</span>
                            <span className="block text-[10px] text-slate-400 font-medium mt-0.5">Problem: {t.reportedProblem}</span>
                          </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-400">
              ...
              <h3 className="font-bold text-slate-700 text-base mt-2">Select a Customer</h3>
              <p className="text-xs text-slate-400 mt-0.5">Select a customer from the left directory to manage profile details.</p>
            </div>
          )}
        </div>
      </div>
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-white rounded-2xl p-8 max-w-md w-full z-10 shadow-2xl relative animate-in zoom-in-95 duration-200">
            <h3 className="font-bold text-xl font-outfit text-slate-900 mb-6">Register Customer</h3>
            ...
            <form onSubmit={createForm.handleSubmit((data) => createCustomerMutation.mutate(data))} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Primary Phone</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Alternate Phone (Opt)</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Residential Address</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">General Notes</label>
                <textarea
