...
export default function ReportsPage() {
  ...
  const isAuthorized = ["SYSTEM_ADMIN", "OWNER", "BRANCH_MANAGER"].includes(
    user.role,
  );
  ...
  const { data: revenueData, isLoading: loadingRevenue } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get("/reports/revenue", {
        params: { startDate, endDate, branchId: activeBranchId },
      }),
  ...
  const handleExportCsv = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `repairflow-report-${new Date().toISOString().split("T")[0]}.csv`,
      );
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 bg-white border border-slate-200 rounded-2xl">
        ...
        <p className="text-sm text-slate-400 mt-1 max-w-sm text-center">
          Operations reports are restricted to shop Managers, Owners, and
          Administrators.
        </p>
      </div>
    );
  ...
  const chartData =
    revenueData?.data?.dailyRevenue?.map((d: any) => ({
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Review financial performance, staff workloads, and turnover
            statistics.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {loadingRevenue ? (
            ...
            <div className="h-64 flex flex-col items-center justify-center text-slate-400">
              ...
              <span className="text-sm">
                No revenue recorded within selected dates
              </span>
            </div>
          )}
        </div>
        ...
        <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-md flex flex-col justify-between relative overflow-hidden">
          <div className="space-y-4">
            ...
            <div>
              <h4 className="text-sm font-semibold text-slate-400 font-outfit uppercase tracking-wider">
                Total Reconciled Profit
              </h4>
              <span className="text-3xl font-extrabold font-outfit mt-1 block">
                {loadingRevenue
                  ...
                  : new Intl.NumberFormat("en-US", {
                      style: "currency",
                      currency: "USD",
                    }).format((revenueData?.data?.totalAmount || 0) / 100)}
              </span>
            </div>
          </div>
          <div className="text-xs text-slate-400 pt-4 border-t border-slate-800 mt-6">
            Computed from cleared invoices only. Refunds and discounts have been
            subtracted.
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-4">
          <h3 className="font-bold text-slate-900 font-outfit flex items-center gap-2">
            <Users className="w-4 h-4 text-slate-400" /> Technician Bench
            Productivity
          </h3>
          <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
            {loadingTechs ? (
              ...
              techsData.data.map((t: any) => (
                <div
                  key={t.id}
                  className="p-4 bg-slate-50 border border-slate-100 rounded-xl flex justify-between items-center text-xs font-semibold text-slate-700"
                >
                  <div>
        ...
        <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-4">
          <h3 className="font-bold text-slate-900 font-outfit flex items-center gap-2 text-rose-700">
            <AlertTriangle className="w-4 h-4 text-rose-500" /> Past Due
            expected repairs
          </h3>
          <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
            {loadingDelayed ? (
              ...
              delayedData.data.map((t: any) => (
                ...
                >
                  <div>
                    <span className="block text-slate-850 font-extrabold font-mono">
                      {t.ticketNumber}
                    </span>
                    <span className="block text-[10px] text-slate-450 font-normal mt-0.5">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-slate-800 font-bold">
                      {new Date(t.expectedCompletionAt).toLocaleDateString()}
                    </span>
                  </div>
                </div>
              ))
