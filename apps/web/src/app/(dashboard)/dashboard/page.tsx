...
export default function DashboardPage() {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-28 bg-slate-100 rounded-2xl border border-slate-200 animate-pulse"
            />
          ))}
        </div>
    ...
    return (
      <div className="bg-red-50 border border-red-200 text-red-700 p-6 rounded-2xl">
        ...
        <p className="text-sm mt-1">
          Please try refreshing the page or check your database seeding.
        </p>
      </div>
    );
  }
  ...
  const dashboardCards = [
    ...
    },
    {
      title: "Revenue (Month)",
      value:
        user.role !== "TECHNICIAN" ? formatMoney(stats.revenueMonth) : "N/A",
      icon: TrendingUp,
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Hello, {user.fullName}. Here is your operations overview.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {dashboardCards.map((card) => {
          ...
          return (
            <div
              key={card.title}
              className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-start justify-between hover-slide"
            >
              <div className="space-y-2">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">
                  {card.title}
                </span>
                <span className="text-2xl font-bold font-outfit text-slate-900 block">
                  {card.value}
                </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm lg:col-span-2">
          <h3 className="font-bold text-slate-900 font-outfit mb-6">
            Repair Ticket Status Distribution
          </h3>
          {chartData.length > 0 ? (
            <div className="h-64">
              ...
              {stats.technicianWorkload.length > 0 ? (
                stats.technicianWorkload.map((tech: any) => (
                  <div
                    ...
                    className="flex justify-between items-center p-3.5 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors"
                  >
