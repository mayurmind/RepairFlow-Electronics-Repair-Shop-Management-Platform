...
import { createTicketSchema, updateTicketStatusSchema, createDiagnosisSchema } from '@repairflow/validation';
...
export default function TicketsPage() {
  ...
  const { data: attachmentsRes } = useQuery<any>({
    ...
    queryFn: () => apiClient.get(`/repair-tickets/${selectedTicket.id}/attachments`),
  ...
  const uploadAttachmentMutation = useMutation({
    mutationFn: async (formData: FormData) => {
      return apiClient.post(`/repair-tickets/${selectedTicket.id}/attachments`, formData, {
        headers: {
  ...
  const { data: ticketsData, isLoading: loadingTickets } = useQuery<any>({
    queryKey: ['tickets', activeBranchId, statusFilter, priorityFilter, assignmentFilter, searchTerm],
    queryFn: async () => {
      const params: any = {};
  ...
  const { data: customerDevicesData } = useQuery<any>({
    ...
    queryFn: () => apiClient.get(`/devices`, { params: { customerId: selectedCustId } }),
  ...
  const assignTechMutation = useMutation({
    mutationFn: (data: { ticketId: string; technicianId: string }) =>
      apiClient.post(`/repair-tickets/${data.ticketId}/assign`, { technicianId: data.technicianId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tickets'] });
      toast.success('Technician assigned successfully!');
      setIsAssignModalOpen(false);
      if (selectedTicket) {
        setSelectedTicket((prev: any) => ({ ...prev, technicianId: selectedTechId }));
      }
    },
    onError: (err: any) => {
  ...
  const submitDiagnosisMutation = useMutation({
    mutationFn: (data: { ticketId: string; diagnosis: any }) =>
      apiClient.post(`/repair-tickets/${data.ticketId}/diagnose`, data.diagnosis),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tickets'] });
  ...
  const getAllowedTransitions = (status: string) => {
    switch (status) {
      ...
      case 'DIAGNOSING':
        return ['WAITING_FOR_APPROVAL', 'PARTS_REQUIRED', 'UNREPAIRABLE', 'CANCELLED'];
      case 'PARTS_REQUIRED':
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight font-outfit text-slate-900">Repair Tickets</h1>
          <p className="text-sm text-slate-500 mt-1">Manage, diagnose, and track branch electronics repairs.</p>
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
          {['ALL', 'RECEIVED', 'DIAGNOSING', 'WAITING_FOR_APPROVAL', 'APPROVED', 'REPAIR_IN_PROGRESS', 'READY_FOR_COLLECTION', 'DELIVERED', 'PARTS_REQUIRED', 'UNREPAIRABLE', 'CANCELLED'].map((status) => (
            <button
              key={status}
      ...
      {loadingTickets ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-64 bg-slate-100 border border-slate-200 rounded-2xl animate-pulse" />
          ))}
        </div>
          ...
          {ticketsData.map((ticket: any) => (
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <span className="text-xs font-mono font-bold text-slate-400 block">{ticket.ticketNumber}</span>
                  <h3 className="font-bold text-slate-800 text-base mt-1 line-clamp-1">{ticket.device.brand} {ticket.device.model}</h3>
                </div>
              ...
              <div className="space-y-2 mb-6">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-500 font-semibold">
                  ...
                  <span className="truncate line-clamp-1">Problem: {ticket.reportedProblem}</span>
                </div>
                {ticket.technician && (
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-100">
                <span className={`text-[10px] font-extrabold uppercase border px-2.5 py-1 rounded-full ${getStatusBadgeColor(ticket.status)}`}>
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          ...
          <h3 className="font-bold text-slate-800 text-lg mt-3">No tickets found</h3>
          <p className="text-sm text-slate-400 mt-1">Try clearing your filters or create a new ticket intake profile.</p>
        </div>
      )}
      ...
      {isDetailDrawerOpen && selectedTicket && (
        <div className="fixed inset-0 z-50 flex justify-end">
          {/* Overlay */}
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs" onClick={() => setIsDetailDrawerOpen(false)} />
          ...
          <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl p-8 flex flex-col justify-between overflow-y-auto animate-in slide-in-from-right duration-200">
            <div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl mb-6 flex flex-wrap justify-between items-center gap-4">
                <div>
                  ...
                  <span className={`inline-block text-[10px] font-extrabold uppercase border px-2.5 py-1 rounded-full mt-1 ${getStatusBadgeColor(selectedTicket.status)}`}>
                ...
                <div className="flex flex-wrap gap-2">
                  {getAllowedTransitions(selectedTicket.status).map((t) => (
                    <button
                      key={t}
                      onClick={() => {
                        if (t === 'DIAGNOSING' && user.role === 'TECHNICIAN' && selectedTicket.technicianId !== user.id) {
                          toast.error('You must be assigned to this ticket to begin diagnosis.');
                          return;
                        }
                        if (t === 'DIAGNOSING' && !selectedTicket.technicianId) {
                          toast.error('Please assign a technician before starting diagnosis.');
                          return;
                        }
                        
                        // If diagnosing, open diagnostic modal
                        if (t === 'WAITING_FOR_APPROVAL' || (t === 'DIAGNOSING' && selectedTicket.status === 'PARTS_REQUIRED')) {
                          diagnosisForm.setValue('repairFeasibility', 'REPAIRABLE');
                  ...
                  {user.role !== 'TECHNICIAN' && !selectedTicket.technicianId && (
                    <button
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-50 pb-1 mb-2">Device Info</h4>
                  <div className="grid grid-cols-2 gap-4 text-sm font-semibold">
                    <div>
                      <span className="text-xs text-slate-400 block font-normal">Serial Number</span>
                      <span>{selectedTicket.device.serialNumber || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-xs text-slate-400 block font-normal">Colour / Variant</span>
                      <span>{selectedTicket.device.colour || 'N/A'} ({selectedTicket.device.variant || 'N/A'})</span>
                    </div>
                    <div className="col-span-2">
                      <span className="text-xs text-slate-400 block font-normal">Cosmetic Damage Intake Notes</span>
                      <p className="font-normal text-slate-700 bg-slate-50 p-3 rounded-lg border border-slate-100 mt-1">{selectedTicket.existingDamage || 'No damage noted at check-in.'}</p>
                    </div>
                  </div>
                </div>

                <div>
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-50 pb-1 mb-2">Customer Info</h4>
                  <div className="text-sm font-semibold">
                    <div>{selectedTicket.customer.fullName}</div>
                    <div className="text-xs text-slate-400 font-normal mt-0.5">Phone: {selectedTicket.customer.phone} | Email: {selectedTicket.customer.email || 'N/A'}</div>
                  </div>
                </div>

                {/* Diagnostics details */}
                {selectedTicket.diagnoses && selectedTicket.diagnoses.length > 0 && (
                  <div>
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-50 pb-1 mb-2">Diagnostic Findings</h4>
                    <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-4 space-y-3 text-sm">
                      {selectedTicket.diagnoses.map((d: any, index: number) => (
                        <div key={d.id} className={index > 0 ? 'pt-3 border-t border-indigo-100' : ''}>
                          <div className="flex justify-between items-center mb-1">
                            <span className="font-extrabold text-indigo-900">{d.faultCategory}</span>
                            <span className="text-[10px] text-indigo-600 font-bold bg-indigo-100/50 px-2 py-0.5 rounded">{d.repairFeasibility}</span>
                          </div>
                          <p className="text-slate-700 text-xs">{d.diagnosticFindings}</p>
                          <div className="text-[10px] text-slate-400 mt-2 font-medium">Recorded by {d.technician?.fullName || 'Staff'} on {new Date(d.createdAt).toLocaleDateString()}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {/* Attachments Section */}
                <div>
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-50 pb-1 mb-2">Ticket Attachments</h4>
            ...
            {user.role !== 'TECHNICIAN' && selectedTicket.status === 'APPROVED' && (
              <div className="pt-6 border-t border-slate-100 flex justify-end">
                <Link href="/estimates" className="flex items-center gap-2 bg-blue-600 text-white font-semibold text-xs px-4 py-2.5 rounded-xl hover:bg-blue-500 transition-colors">
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs" onClick={() => setIsCreateModalOpen(false)} />
          <div className="bg-white rounded-2xl p-8 max-w-xl w-full z-10 shadow-2xl relative max-h-[85vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            <h3 className="font-bold text-xl font-outfit text-slate-900 mb-6">Create Repair Ticket</h3>

            <form onSubmit={createForm.handleSubmit((data) => createTicketMutation.mutate(data))} className="space-y-4">
              {/* Customer */}
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Select Customer</label>
                <select
                ...
                {createForm.formState.errors.customerId && (
                  <span className="text-red-500 text-xs mt-1 block">{createForm.formState.errors.customerId.message}</span>
                )}
              </div>

              {/* Device */}
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Select Customer Device</label>
                <select
                ...
                {createForm.formState.errors.deviceId && (
                  <span className="text-red-500 text-xs mt-1 block">{createForm.formState.errors.deviceId.message}</span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Expected Completion Date</label>
                  <input
                    type="date"
              ...
              {/* Problem Problem */}
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Reported Problem</label>
                <textarea
                ...
                {createForm.formState.errors.reportedProblem && (
                  <span className="text-red-500 text-xs mt-1 block">{createForm.formState.errors.reportedProblem.message}</span>
                )}
              </div>

              {/* Cosmetic damage */}
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Pre-existing cosmetic damage / accessories</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Initial Public Note</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Initial Internal Note</label>
                  <input
                    type="text"
      ...
      {isAssignModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs" onClick={() => setIsAssignModalOpen(false)} />
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full z-10 shadow-2xl relative animate-in zoom-in-95 duration-200">
            <h3 className="font-bold text-lg font-outfit text-slate-900 mb-4">Assign Technician</h3>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Technicians List</label>
                <select
      ...
      {isDiagnosisModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs" onClick={() => setIsDiagnosisModalOpen(false)} />
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full z-10 shadow-2xl relative max-h-[80vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            <h3 className="font-bold text-lg font-outfit text-slate-900 mb-4">Submit Diagnosis Findings</h3>
            ...
            >
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Fault Category</label>
                <input
                  type="text"
                ...
                {diagnosisForm.formState.errors.faultCategory && (
                  <span className="text-red-500 text-xs mt-1 block">{diagnosisForm.formState.errors.faultCategory.message}</span>
                )}
              </div>

              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Diagnostic Findings</label>
                <textarea
                ...
                {diagnosisForm.formState.errors.diagnosticFindings && (
                  <span className="text-red-500 text-xs mt-1 block">{diagnosisForm.formState.errors.diagnosticFindings.message}</span>
                )}
              </div>

              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Recommended Repair</label>
                <textarea
                ...
                {diagnosisForm.formState.errors.recommendedRepair && (
                  <span className="text-red-500 text-xs mt-1 block">{diagnosisForm.formState.errors.recommendedRepair.message}</span>
                )}
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Feasibility</label>
                  <select
                  ...
                  >
                    ...
                    <option value="PARTIALLY_REPAIRABLE">Partially Repairable</option>
                    <option value="UNREPAIRABLE">Unrepairable</option>
                    <option value="FURTHER_TESTING_REQUIRED">Further Testing Required</option>
                  </select>
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Parts Needed</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Labor / Time Description</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Public Explanation</label>
                  <textarea
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Internal Notes</label>
                  <textarea
      ...
      {isStatusModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs" onClick={() => setIsStatusModalOpen(false)} />
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full z-10 shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            >
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Public Note (Optional)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Internal Note (Optional)</label>
                <input
                  type="text"
