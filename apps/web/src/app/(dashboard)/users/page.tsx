    ...
    onSuccess: async (res: any) => {
      ...
      if (selectedBranchIds.length > 0 && res?.data?.id) {
        for (const bId of selectedBranchIds) {
          await apiClient.post(`/users/${res.data.id}/branches`, {
            branchId: bId,
          });
        }
      }
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 bg-white border border-slate-200 rounded-2xl">
        ...
        <p className="text-sm text-slate-400 mt-1 max-w-sm text-center">
          Staff account settings are restricted to managers, owners, and system
          administrators.
        </p>
      </div>
    );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Register staff profiles, modify roles, and assign localized branch
            mappings.
          </p>
        </div>
      ...
      {loadingUsers ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-56 bg-slate-100 border border-slate-200 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : staffList.length > 0 ? (
              ...
              <div>
                <div className="flex justify-between items-start mb-4">
                  ...
                  {user.role !== "BRANCH_MANAGER" && staff.id !== user.id ? (
                    ...
                    <span
                      className={`text-[10px] font-extrabold uppercase border px-2 py-0.5 rounded-full ${
                        staff.status === "ACTIVE"
