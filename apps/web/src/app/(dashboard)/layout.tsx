...
export default function DashboardLayout({
  ...
  const { user, loading, activeBranchId, setActiveBranchId, logout } =
    useAuth();
  ...
  useEffect(() => {
    console.log(
      "DashboardLayout: useEffect check - loading =",
      loading,
      "user =",
      user,
    );
    if (!loading && !user) {
      console.log(
        "DashboardLayout: No session found. Redirecting to /login...",
      );
      router.push("/login");
    }
  }, [user, loading, router]);
  ...
  console.log(
    "DashboardLayout: Render state - loading =",
    loading,
    "user =",
    user,
  );
            ...
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              {sidebarExpanded && (
                <div className="transition-all duration-200">
                  <span className="font-bold text-lg tracking-tight text-slate-900 font-outfit block">
                    RepairFlow
                  </span>
                  <span className="text-[9px] block text-slate-400 font-mono tracking-widest uppercase">
                    Console
                  </span>
                </div>
              )}
                      ...
                      notifications.map((n: any) => (
                        ...
                        >
                          <div className="min-w-0 flex-1">
                            <p
                              className={`text-xs font-semibold text-slate-900 ${!n.isRead ? "font-bold" : ""}`}
                            >
                              {n.title}
                            </p>
