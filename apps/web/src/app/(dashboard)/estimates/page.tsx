...
export default function EstimatesPage() {
  ...
  const { data: ticketsData } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get("/repair-tickets", {
        params: { branchId: activeBranchId, status: "WAITING_FOR_APPROVAL" },
      }),
  ...
  const createMutation = useMutation({
    mutationFn: (data: { ticketId: string; payload: any }) =>
      apiClient.post(
        `/repair-tickets/${data.ticketId}/estimates`,
        data.payload,
      ),
    onSuccess: () => {
      ...
      createForm.reset({
        items: [
          { itemType: "PART", description: "", quantity: 1, unitPrice: 0 },
        ],
        validUntil: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0],
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 bg-white border border-slate-200 rounded-2xl">
        ...
        <p className="text-sm text-slate-400 mt-1 max-w-sm text-center">
          Technicians are restricted from accessing financial estimates or
          customer contracts.
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage draft quotations, release customer links, and track approval
            actions.
          </p>
        </div>
        <button
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-wrap gap-2">
        {["ALL", "DRAFT", "SENT", "APPROVED", "REJECTED", "CANCELLED"].map(
          (st) => (
            <button
              key={st}
      ...
      {loadingEstimates ? (
            ...
            >
              <div>
                ...
                <h3 className="font-bold text-slate-800 text-base">
                  {est.repairTicket.device.brand}{" "}
                  {est.repairTicket.device.model}
                </h3>
                ...
                <div className="space-y-1.5 mt-4 text-xs font-semibold text-slate-400">
                  <div>
                    ...
                    <span className="text-slate-600 font-bold">
                      {est.repairTicket.customer.fullName}
                    </span>
                  </div>
                  <div>
                    Ticket:{" "}
                    <span className="text-slate-600 font-mono">
                      {est.repairTicket.ticketNumber}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-sm text-slate-400 mt-1">
            Compile diagnostics on ready tickets to generate client estimates.
          </p>
        </div>
      )}
            ...
            <div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden">
                    {selectedEstimate.items?.map((item: any) => (
                      <div
                        key={item.id}
                        className="p-3 bg-slate-50/50 flex justify-between items-center text-xs font-semibold"
                      >
                        <div>
                    ...
                    <div className="p-3 flex justify-between items-center text-xs font-bold text-slate-500 bg-slate-100/50">
                      ...
                      <span className="text-slate-900 font-extrabold text-sm">
                        {new Intl.NumberFormat("en-US", {
                          style: "currency",
                          currency: "USD",
                        }).format(selectedEstimate.totalAmount / 100)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {selectedEstimate.status === "SENT" &&
                  selectedEstimate.approvalToken && (
                    <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 space-y-3">
                      <span className="text-xs font-bold text-purple-900 block">
                        Customer Direct Interface
                      </span>
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            copyApprovalLink(selectedEstimate.approvalToken)
                          }
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
                  {fields.map((field, index) => (
                    <div
                      key={field.id}
                      className="grid grid-cols-12 gap-2 items-center bg-slate-50 p-3 rounded-xl border border-slate-100"
                    >
