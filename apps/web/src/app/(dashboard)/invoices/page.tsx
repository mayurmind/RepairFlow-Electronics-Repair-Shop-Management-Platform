...
export default function InvoicesPage() {
  ...
  const { data: ticketsData } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get("/repair-tickets", {
        params: { branchId: activeBranchId, status: "READY_FOR_COLLECTION" },
      }),
    ...
    onSuccess: (res: any) => {
      ...
      if (selectedInvoice) {
        setSelectedInvoice((prev: any) => ({
          ...
          status:
            prev.balanceDue - res.amountPaid === 0 ? "PAID" : "PARTIALLY_PAID",
        }));
      }
    },
    onError: (err: any) => {
  ...
  const voidMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error(
        err.message || "Paid or partially paid invoices cannot be voided.",
      );
    },
  });
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 bg-white border border-slate-200 rounded-2xl">
        ...
        <p className="text-sm text-slate-400 mt-1 max-w-sm text-center">
          Technicians are restricted from accessing financial ledgers or client
          billing statements.
        </p>
      </div>
    );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Process invoice receipts, capture payments, and print customer
            copies.
          </p>
        </div>
        <button
      ...
      {loadingInvoices ? (
            ...
            >
              <div>
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span
                    className={`text-[9px] px-2.5 py-0.5 rounded-full font-extrabold border uppercase ${
                      inv.status === "PAID"
                ...
                <h3 className="font-bold text-slate-800 text-base">
                  {inv.repairTicket.device.brand}{" "}
                  {inv.repairTicket.device.model}
                </h3>
                ...
                <div className="space-y-1.5 mt-4 text-xs font-semibold text-slate-400">
                  <div>
                    ...
                    <span className="text-slate-700 font-bold">
                      {inv.repairTicket.customer.fullName}
                    </span>
                  </div>
                  <div>
                    Ticket:{" "}
                    <span className="text-slate-750 font-mono">
                      {inv.repairTicket.ticketNumber}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex justify-between items-center">
                <div>
                  <span className="text-[9px] font-medium text-slate-450 block uppercase">
                    Balance Due
                  </span>
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-sm text-slate-400 mt-1">
            Invoice generation triggers when ticket status becomes ready for
            collection.
          </p>
        </div>
      )}
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-slate-800 font-bold">
                        {new Intl.NumberFormat("en-US", {
                          style: "currency",
                          currency: "USD",
                        }).format(selectedInvoice.subtotalAmount / 100)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="text-red-600 font-bold">
                        ...
                        {new Intl.NumberFormat("en-US", {
                          style: "currency",
                          currency: "USD",
                        }).format(selectedInvoice.discountAmount / 100)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="text-slate-800 font-bold">
                        {new Intl.NumberFormat("en-US", {
                          style: "currency",
                          currency: "USD",
                        }).format(selectedInvoice.taxAmount / 100)}
                      </span>
                    </div>
                    <div className="flex justify-between border-t border-slate-200 pt-2.5 text-slate-900 text-sm font-bold">
                      ...
                      <span className="text-slate-950 font-extrabold">
                        {new Intl.NumberFormat("en-US", {
                          style: "currency",
                          currency: "USD",
                        }).format(selectedInvoice.totalAmount / 100)}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between text-xs font-bold text-slate-700">
                      ...
                      <span className="text-slate-900 font-bold">
                        {new Intl.NumberFormat("en-US", {
                          style: "currency",
                          currency: "USD",
                        }).format(selectedInvoice.balanceDue / 100)}
                      </span>
                    </div>
                  </div>
                </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {selectedInvoice.payments.map((p: any) => (
                          <div
                            key={p.id}
                            className="p-3 bg-slate-50 border border-slate-100 rounded-xl flex justify-between items-center text-xs font-semibold text-slate-700"
                          >
                            <div>
                              <span className="block text-slate-850 font-extrabold">
                                {p.method} Payment
                              </span>
                              {p.referenceNumber && (
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-white rounded-2xl p-8 max-w-md w-full z-10 shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            <form
              onSubmit={createInvoiceForm.handleSubmit(handleCreateSubmit)}
              className="space-y-4"
            >
              {/* Select Ticket */}
              <div>
                ...
                >
                  ...
                  {ticketsData?.map((t: any) => (
                    <option key={t.id} value={t.id}>
                      {t.ticketNumber} - {t.customer.fullName} ({t.device.brand}{" "}
                      {t.device.model})
                    </option>
                  ))}
                </select>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <span className="text-[10px] text-slate-400 block mt-1">
                    Max limit: FrontDesk $20, Manager $50
                  </span>
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full z-10 shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            >
              <div>
                ...
                />
                <span className="text-[10px] text-slate-400 block mt-1">
                  Remaining balance: ${selectedInvoice.balanceDue / 100}
                </span>
