...
export default function PublicEstimateApprovalPage() {
  ...
  const mutation = useMutation({
    mutationFn: (vars: { decision: 'APPROVED' | 'REJECTED'; customerComment: string }) =>
      apiClient.post(`/public/estimates/${token}/${vars.decision.toLowerCase()}`, {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-sm text-slate-500 font-medium font-outfit">Retrieving estimate breakdown...</span>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-premium p-8 text-center">
          ...
          <h2 className="font-bold text-xl text-slate-900 font-outfit">Estimate Review Failed</h2>
          <p className="text-sm text-slate-500 mt-2">
            The estimate link is expired, voided, or does not exist. Please contact support.
          </p>
          <Link
            href="/"
  ...
  if (decisionMade || estimate.status === 'APPROVED' || estimate.status === 'REJECTED') {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-premium p-8 text-center relative overflow-hidden">
          {/* Top colored strip */}
          <div className={`absolute top-0 left-0 right-0 h-1.5 ${isApproved ? 'bg-green-600' : 'bg-red-500'}`} />
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between">
      ...
      <header className="border-b border-slate-200 bg-white/70 backdrop-blur-md sticky top-0 z-50 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] block text-slate-500 uppercase tracking-widest font-mono">Estimate Center</span>
          </div>
        </div>
        <Link
          href={`/track/${token}`}
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 flex-grow w-full space-y-6">
        ...
        {/* Intro */}
        <div>
          <h1 className="text-3xl font-extrabold font-outfit text-slate-950">Review Repair Estimate</h1>
          <p className="text-sm text-slate-500 mt-1">
            Please inspect the diagnostic line items below compiled for Ticket Ref: <span className="font-bold text-slate-800">{estimate.ticket.ticketNumber}</span>
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-premium overflow-hidden">
          ...
          <div className="bg-slate-50 border-b border-slate-100 p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-slate-900 font-outfit">{estimate.estimateNumber}</h4>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono">Cost Estimate</span>
              </div>
            </div>
          ...
          <div className="p-6">
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                {estimate.items.map((item: any, idx: number) => (
                  <tr key={idx}>
                    ...
                    <td className="py-4 text-right text-slate-900">{formatMoney(item.totalPrice)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="border-t border-slate-100 mt-6 pt-6 flex flex-col items-end gap-2.5 text-sm">
              ...
              {estimate.discountAmount > 0 && (
                <div className="flex justify-between w-64 text-slate-500 font-medium">
                  <span>Discount:</span>
                  <span className="text-green-600">-{formatMoney(estimate.discountAmount)}</span>
                </div>
              )}
              <div className="flex justify-between w-64 text-base font-bold text-slate-950 border-t border-slate-100 pt-3">
                <span>Total Amount:</span>
                <span className="text-blue-600">{formatMoney(estimate.totalAmount)}</span>
              </div>
            </div>
          </div>
        </div>
