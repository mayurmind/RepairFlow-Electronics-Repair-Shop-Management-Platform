...
test("Phase 2 core workflow E2E test", async ({ page }) => {
  ...
  await page.fill('input[placeholder="e.g. customer@gmail.com"]', `e2e_${randSuffix}@gmail.com`);
  await page.fill('input[placeholder="Street details, city, postal code"]', "123 E2E Lane");
  await page.click('button[type="submit"]:has-text("Register")');

  // Verify customer is in the list
  const customerItem = page.locator(`.flex-1.overflow-y-auto.divide-y button:has-text("${customerName}")`);
  ...
  await page.fill('input[placeholder="e.g. Phone, Tablet, Laptop, Console"]', "Mobile phone");
  await page.fill('input[placeholder="e.g. Apple, Samsung"]', "Google");
  await page.fill('input[placeholder="e.g. iPhone 15 Pro"]', "Pixel 8");
  await page.fill('input[placeholder="e.g. DX4F82..."]', `SN-E2E-${randSuffix}`);
  await page.click('button[type="submit"]:has-text("Register")');
  ...
  await expect(page.locator(`.grid :has-text("${customerName}")`).first()).toBeVisible();
  ...
  await expect(page.locator('text=Technician assigned successfully!')).toBeVisible();
  ...
  await page.fill('input[placeholder="Explain status change to customer..."]', "Starting diagnosis");
  await page.fill('input[placeholder="Private internal log comments..."]', "Diagnosing board rails");
  await page.click('button:has-text("Confirm Change")');
  await expect(page.locator('text=Ticket status updated successfully!')).toBeVisible();

  // Record diagnosis and transition to WAITING_FOR_APPROVAL
  await page.click("#status-btn-WAITING_FOR_APPROVAL");
  await page.fill('input[placeholder="e.g. Screen Replacement, Battery Degradation"]', "Screen Damage");
  await page.fill('textarea[placeholder="What is wrong with the device?"]', "Primary display digitizer is shattered");
  await page.fill('textarea[placeholder="What steps are required to resolve?"]', "Replace full front glass and OLED module");
  await page.selectOption('select:has-text("Choose Feasibility")', { label: "Repairable" });
  await page.click('button:has-text("Submit findings")');
  await expect(page.locator('text=Diagnosis recorded successfully!')).toBeVisible();

  // Confirm timeline contains the updates
  await page.click("text=Timeline");
  await expect(page.locator(".relative.border-l")).toContainText("DIAGNOSING");
  await expect(page.locator(".relative.border-l")).toContainText("WAITING_FOR_APPROVAL");
});
